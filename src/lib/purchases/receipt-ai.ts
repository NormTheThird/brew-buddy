...
- KITS: if a line item is a true kit or starter set (multiple independent products), determine its contents and expand it into one row per component with the right kind/category/type and "partOfKit" set to the kit's name.
- KIT LOOKUP PRECISION: search the VENDOR'S OWN website product page for the exact kit FIRST (e.g. site:northernbrewer.com + the kit name) and use its stated contents and amounts VERBATIM. Third-party sources only if the vendor page can't be found, and only for items they explicitly list. Precision beats completeness. Do NOT emit a row for the kit container itself, and do NOT invent per-component prices (the kit's price stays at the purchase level). If you cannot determine the contents, fall back to a single row for the kit with no partOfKit.
...
- LOOKED-UP CONTENT MUST BE CONFIRMED: only list kit components the vendor's listing EXPLICITLY states. If the listing doesn't itemize something (e.g. exact steeping-grain varieties), use ONE generic row ("Kit steeping grains") with no invented varieties or amounts. Never present a plausible guess as a component.`;
}
